import sys
from src.logger import logging

def error_message_details(error, error_detail:sys):
    # Fetching the error details using sys
    _,_,exc_tb=error_detail.exc_info()
    # Fetaching the filename where the error occured
    file_name = exc_tb.tb_frame.f_code.co_filename
    # Variable storing message providing filename, line number and error message.
    error_message= "Error occured in python script name [{0}] line number [{1} error_message[{2}]]".format(
        file_name, exc_tb.tb_lineno, str(error))
    # Returing error message
    return error_message

class CustomException(Exception):
    def __init__(self, error_message, error_detail: sys):
        super().__init__(error_message)
        # Error message that is tracked and return by sys
        self.error_messsage=error_message_details(error_message, error_detail=error_detail)

    def __str__(self):
        return self.error_messsage
